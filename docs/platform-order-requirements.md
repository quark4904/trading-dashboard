...
실행 엔진은 주문 계획을 만든 뒤 다음 검사를 모두 통과해야 `dry_run` 주문을 기록한다.

- 업비트: `orders/chance`의 활성 상태, `price` 주문 지원, 매수 가능 원화, 최소·최대 주문 금액
- 주식 플랫폼: 최근 성공한 잔고 동기화의 통화별 주문 가능 현금과 수량 주문의 현재가
- 플랫폼별 국내/미국 정규 거래시간
- 전략의 일일 원화 예산과 최대 주문 횟수
- 주문별 36자 이내 멱등성 키 중복 여부

검증에 실패하면 외부 주문 API를 호출하지 않고 `risk_rejected` 상태의 주문과 실행 이력 오류를
저장한다. 현재는 실주문 어댑터가 없으므로 취소 정책은 `reject_before_submission`으로
기록되며, 향후 실주문 전환 시 미체결 주문 취소 정책의 기준으로 사용한다.
