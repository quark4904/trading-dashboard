...
토스증권, 한국투자증권, 업비트 연동을 목표로 한 DCA 전용 자동매매 대시보드 MVP입니다.
...
- DCA 전략 추가 및 활성/중지 관리
- DCA 전략 실행 로그 조회
- SQLite 기반 DCA 전략/실행 기록 저장
...
실제 체결 이력은 계좌 기준의 전체 체결 내역이므로 거래소·증권사 앱에서 직접 실행한 거래도
포함될 수 있습니다. 동기화로 가져온 체결은 `외부 체결`로 표시하고, 실제 전략 주문과
`strategy_id`·`strategy_run_id`가 연결된 경우에만 `전략 연결`로 표시합니다. 화면의 체결 출처
필터로 두 종류를 분리해서 볼 수 있으며, `전략 실행 이력 (이 앱)`과 `DRY_RUN 생성 주문 (이 앱)`은
이 앱이 만든 실행 기록만 표시합니다.
...
새 데이터베이스는 기본적으로 빈 상태로 생성됩니다. 개발용 샘플 자산과 DCA 전략이 필요할 때만 아래 환경 변수를 설정합니다.
...
1. 모의매매(가상 잔고·체결 상태 영속화)
2. FastAPI + React 전환
3. 제한된 실주문 전환

백테스트 API의 요청 형식과 현재 제한사항은
[`docs/backtesting.md`](docs/backtesting.md)를 참고하세요.
